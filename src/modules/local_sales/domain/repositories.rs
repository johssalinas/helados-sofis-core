...
pub trait LocalSaleRepository: Send + Sync {
    ...
    async fn find_by_id_with_items(&self, id: Uuid) -> Result<Option<LocalSaleWithItems>, AppError>;
    async fn find_todays(&self) -> Result<Vec<LocalSale>, AppError>;
    /// Crea venta, resta inventario, registra en caja. Todo transaccional.
    async fn create_sale(&self, dto: &CreateLocalSaleDto, created_by: Uuid) -> Result<LocalSale, AppError>;
}
